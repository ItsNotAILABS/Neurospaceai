...
function updateConnectionWeights(registry, neuralState) {
  ...
  const updated = registry.connections.map((c) => {
    ...
    return {
      ...c,
      weight: newWeight,
      activationFreq: running ? Math.min(0.99, c.activationFreq + 1e-3) : c.activationFreq
    };
  });
  return { ...registry, connections: updated, lastUpdated: Date.now() };
}
function getOptimizationRecommendations(registry) {
  const recs = [];
  for (const c of registry.connections) {
    if (c.usefulness < 0.55 && c.failureAssociation > 0.25) {
      ...
      recs.push({
        connectionId: c.id,
        action: "promote",
        reason: "High utility + reliability — candidate for structural promotion",
...
export {
  createConnectionRegistry as c,
  getOptimizationRecommendations as g,
  updateConnectionWeights as u
};
