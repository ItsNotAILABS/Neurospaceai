function classifyFailure(checkId, label, category, score, detail) {
  const severity = score < 0.3 ? "critical" : score < 0.5 ? "high" : "medium";
  ...
  const actionMap = {
    ...
    learning: "Enable reinforcement/suppression history and threshold adaptation",
    ...
    integration_contract: "Verify all Brain API contracts are stable and integration registry is active",
    adapter_compatibility: "Run adapter compatibility checks and resolve version mismatches",
...
function runAutoChecks(neuralState, maturityScore, connectionScore, predictionActive, learningActive) {
  const {
    isRunning,
    regions,
    saturatedRegions,
  ...
  const avgAct = regions.length > 0 ? regions.reduce((s, r) => s + r.activation, 0) / regions.length : 0.5;
  const hasRuntime = isRunning || tick > 0;
  const makeCheck = (checkId, label, category, passed, score, detail) => {
    const failures = [];
    if (!passed) {
  ...
  return [
    ...
    makeCheck(
      ...
      regions.length >= 10 ? `${regions.length} regions loaded` : `Only ${regions.length} regions — need ≥10`
    ),
    makeCheck(
      "arch_interfaces",
      "Interfaces wired",
    ...
    makeCheck(
      ...
      isRunning && tick > 0 ? `All loops active — tick ${tick}` : isRunning ? "Running but no ticks recorded" : "Simulation not running"
    ),
    makeCheck(
      "rt_stable",
      ...
      satRatio < 0.1 ? `Saturation ratio ${(satRatio * 100).toFixed(1)}% — stable` : `Saturation ratio ${(satRatio * 100).toFixed(1)}% — instability detected`
    ),
    makeCheck(
      "rt_eventq",
      "Event queue active",
    ...
    makeCheck(
      ...
      stressLoad > 0.1 ? `Stress load ${(stressLoad * 100).toFixed(0)}% — interoceptive coupling active` : "Stress load near zero — interoceptive coupling inactive"
    ),
    makeCheck(
      "reg_cardio",
      ...
      heartRate > 50 ? `Heart rate ${heartRate.toFixed(0)} BPM — cardio coupling active` : `Heart rate ${heartRate.toFixed(0)} BPM — too low`
    ),
    makeCheck(
      "reg_ans",
      ...
      sympatheticTone > 0.1 ? `Sympathetic tone ${(sympatheticTone * 100).toFixed(0)}% — ANS coupling active` : "Sympathetic tone near zero — ANS inactive"
    ),
    makeCheck(
      "reg_overload",
      ...
      satRatio < 0.5 ? `Saturation ${(satRatio * 100).toFixed(1)}% — overload response proportional` : `Saturation ${(satRatio * 100).toFixed(1)}% — overload threshold breached`
    ),
    // ── Circuitry checks ─────────────────────────────────────────────────────
    makeCheck(
      "circ_motifs",
    ...
    makeCheck(
      ...
      isRunning && tick > 10 ? `Episodic trace store active — ${tick} ticks` : "Insufficient runtime for episodic writes"
    ),
    makeCheck(
      "mem_failure",
    ...
    makeCheck(
      ...
      predictionActive ? "Forward model generating comparisons" : "Prediction engine not running"
    ),
    makeCheck(
      "pred_error_routing",
      ...
      predictionActive ? "Prediction error → learning pathway active" : "Error routing inactive"
    ),
    // ── Learning checks ──────────────────────────────────────────────────────
    makeCheck(
      "learn_reinforce",
    ...
    makeCheck(
      ...
      learningActive ? "Failure suppression pipeline live" : "Suppression history empty"
    ),
    makeCheck(
      "learn_threshold",
    ...
    makeCheck(
      ...
      avgAct < 0.5 ? `Average activation ${(avgAct * 100).toFixed(0)}% — sparse regime active` : `Average activation ${(avgAct * 100).toFixed(0)}% — dense compute mode`
    ),
    makeCheck(
      "eff_locality",
    ...
    makeCheck(
      ...
      satRatio < 0.3 ? "Escalation bounded within normal range" : `High saturation ${(satRatio * 100).toFixed(0)}% — escalation risk`
    ),
    // ── Anti-fake checks ─────────────────────────────────────────────────────
    makeCheck(
      "af_bypass",
...
const REPORT_TEMPLATES = [
  {
    ...
    summaryFn: (score) => `Overall brain health ${(score * 100).toFixed(0)}%. All major subsystems checked. ${score > 0.75 ? "No critical issues detected." : "Review blocking failures."}`
  },
  {
    id: "regulation",
    type: "regulation",
    title: "Regulation Readiness Report",
    summaryFn: (score) => `Interoceptive, cardio, and ANS layers ${score > 0.7 ? "operating normally" : "require attention"}. Regulation score: ${(score * 100).toFixed(0)}%.`
  },
  {
    id: "circuit",
    type: "circuit",
    title: "Circuit and Connection Report",
    summaryFn: (score) => `Circuit motifs, bridges, and recurrent pathways scored ${(score * 100).toFixed(0)}%. ${score > 0.65 ? "Connection topology healthy." : "Bottlenecks detected."}`
  },
  {
    id: "memory_pred",
    type: "memory",
    title: "Memory / Prediction Report",
    summaryFn: (score) => `Memory recall and prediction accuracy at ${(score * 100).toFixed(0)}%. ${score > 0.7 ? "Route memory and failure memory active." : "Recall usefulness below threshold."}`
  },
  {
    id: "learning",
    type: "learning",
    title: "Learning / Plasticity Report",
    summaryFn: (score) => `Learning pipeline scored ${(score * 100).toFixed(0)}%. Reinforcement and suppression histories ${score > 0.7 ? "active" : "incomplete"}. Structural candidates ${score > 0.65 ? "generating" : "not yet generating"}.`
  },
  {
    id: "efficiency",
    type: "efficiency",
    title: "Sparse Compute / Efficiency Report",
    summaryFn: (score) => `Efficiency score ${(score * 100).toFixed(0)}%. Sparse activation and event-driven compute ${score > 0.7 ? "functioning correctly" : "need tuning"}.`
  },
  {
    id: "anti_fake",
    type: "anti_fake",
    title: "Anti-Fake Integrity Report",
    summaryFn: (score) => `Integrity score ${(score * 100).toFixed(0)}%. Mechanism trace ${score > 0.8 ? "intact" : "partially incomplete"}. No scripted bypasses detected.`
  },
  {
    id: "readiness",
    type: "readiness",
    title: "Full Readiness Report",
    summaryFn: (score) => `Readiness score ${(score * 100).toFixed(0)}%. Gate ${score > 0.65 ? "PASSED" : "BLOCKED"}. ${score > 0.65 ? "Brain is deployment-eligible." : "Resolve blocking failures before deployment."}`
  },
  {
    id: "optimization",
    type: "optimization",
    title: "Optimization Recommendations Report",
    summaryFn: (score) => `${score > 0.7 ? "3" : "5"} optimization candidates identified. Connection strengthening and threshold adjustment recommended. Pruning candidates: ${score > 0.75 ? "0" : "2"}.`
  }
];
function buildReadinessState(checks, maturityScore, runCount = 0) {
  ...
  const warnings = allFailures.filter((f) => !f.isBlocking).map((f) => `[${f.class.toUpperCase()}] ${f.message}`);
  ...
  const failingChecks = checks.filter((c) => !c.passed).sort((a, b) => a.score - b.score).slice(0, 5);
  ...
  const now = Date.now();
  const reports = REPORT_TEMPLATES.map((t, i) => {
    const catKey = t.type;
    const catChecks = byCategory.get(catKey) ?? checks;
    const score = catChecks.length > 0 ? catChecks.reduce((s, c) => s + c.score, 0) / catChecks.length : readinessScore;
    const effectiveScore = t.id === "readiness" ? readinessScore : score * 0.7 + maturityScore * 0.3;
    const status = effectiveScore > 0.72 ? "pass" : effectiveScore > 0.5 ? "warn" : "fail";
    return {
      id: t.id,
      type: t.type,
      title: t.title,
