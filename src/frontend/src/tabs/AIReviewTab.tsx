...
import { createArtifact, useArtifacts } from "../utils/artifactStore";
...
type RecStatus = "PENDING" | "IN_PROGRESS" | "IMPLEMENTED";
...
const BASE_RECOMMENDATIONS: Recommendation[] = [
  {
    ...
    impact: "HIGH",
    rationale:
      "Inter-hemispheric weight transfer is the primary bottleneck for emergent bilateral coordination. Low callosal pathway strength limits global brain coherence.",
    action:
      "Increase callosal connection weights by 35%, add bidirectional motif tagging for all callosal arcs.",
    status: "IN_PROGRESS",
    module: "core-circuit",
  },
  {
    ...
    impact: "HIGH",
    rationale:
      "Sustained sympathetic dominance under fatigue suppresses recovery pathways. Regulatory rebound is too slow, causing extended overload states.",
    action:
      "Reduce parasympathetic re-engagement threshold from 0.7 to 0.55 under fatigue > 0.6.",
    status: "PENDING",
  ...
  },
  {
    ...
    impact: "HIGH",
    rationale:
      "Prediction error signals are not consistently amplifying salience for novel stimuli. This reduces the system's ability to prioritize surprise-driven learning.",
    action:
      "Add dedicated prediction-error-to-salience bridge motif with weight 0.8 on mismatch > 0.4.",
    status: "PENDING",
    module: "core-memory",
  },
  {
    ...
    impact: "MED",
    rationale:
      "Compute escalation from local to broad update takes 3-5 ticks under high load. This delays critical arbitration under urgency pressure.",
    action:
      "Implement preemptive escalation trigger at urgency > 0.75 before overload threshold is reached.",
    status: "PENDING",
    module: "core-runtime",
  },
  {
    ...
    impact: "MED",
    rationale:
      "Cardio collapse risk above 0.85 is not sufficiently lowering action selection confidence. The coupling coefficient is too weak for extreme exertion states.",
    action:
      "Increase cardio-to-threshold bridge weight to 0.65 when collapse_risk > 0.8.",
    status: "IN_PROGRESS",
    module: "core-regulation",
  },
  {
    ...
    impact: "MED",
    rationale:
      "Failure memory is stored but insufficiently driving approach/avoid circuit bias. Recent route failures should suppress approach bias in analogous contexts.",
    action:
      "Route failure_memory recall confidence directly into NAC inhibition pathway with weight 0.5.",
    status: "PENDING",
    module: "core-circuit",
  },
  {
    ...
    impact: "MED",
    rationale:
      "Learning rate is currently static per epoch. High-regulation states should suppress plasticity to prevent trauma-driven over-fitting.",
    action:
      "Multiply base learning rate by (1 - regulation_burden * 0.5) on each learning step.",
    status: "IMPLEMENTED",
  ...
  },
  {
    ...
    impact: "LOW",
    rationale:
      "Episodic memory consolidation currently uses temporal proximity. Semantic similarity clustering would improve context-driven recall accuracy.",
    action:
      "Implement cosine similarity clustering pass in the slow-loop consolidation step.",
    status: "PENDING",
    module: "core-memory",
  },
  {
    ...
    impact: "LOW",
    rationale:
      "Current version compatibility check is major-version only. Minor version drift can introduce silent schema mismatches in complex payloads.",
    action:
      "Extend version check to include minor version matching for payload fields with breaking changes.",
    status: "PENDING",
  ...
  },
  {
    ...
    impact: "MED",
    rationale:
      "Working memory gate admits all high-salience items under burst conditions, causing slot exhaustion. Competitive suppression is needed.",
    action:
      "Add lateral inhibition pass after salience ranking before WM admission. Top 5 only.",
    status: "PENDING",
    module: "core-runtime",
  },
];
...
function statusColor(status: RecStatus): string {
  switch (status) {
    case "IMPLEMENTED":
      return GREEN;
    case "IN_PROGRESS":
...
export default function AIReviewTab() {
  ...
  const [recommendations, setRecommendations] =
    useState<Recommendation[]>(BASE_RECOMMENDATIONS);
  const [expanded, setExpanded] = useState<string | null>(null);
  const [applyingId, setApplyingId] = useState<string | null>(null);

  const aiArtifacts = artifacts
    .filter((a) => a.artifact_type === "ai_review")
    .slice(0, 3);
  ...
  const overallScore = Math.round(
    (implementedCount / recommendations.length) * 60 +
      (inProgressCount / recommendations.length) * 25 +
      (highImpactPending === 0 ? 15 : 0),
  );
  ...
  const assessmentColor =
    overallScore >= 60 ? GREEN : overallScore >= 35 ? AMBER : RED;
  ...
  function handleApply(id: string) {
    ...
    setTimeout(() => {
      ...
      createArtifact({
        ...
        source_system: "core",
        title: `Applied: ${recommendations.find((r) => r.id === id)?.title ?? "Recommendation"}`,
        summary:
          "Recommendation applied via AI Review tab. Candidate queued for maturation testing.",
        score: 72,
        status: "info",
        ai_review_summary:
          "Candidate change submitted to validation queue. Evidence-based promotion pending.",
    ...
    >
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <div className="flex items-center gap-3">
              ...
              >
                {overallScore}% optimization progress
              </span>
            </div>
          </div>
                ...
                >
                  ...
                  {rec.status !== "IMPLEMENTED" && (
                    <button
                      ...
                      style={{
                        background:
                          applyingId === rec.id ? `${GREEN}10` : `${CYAN}18`,
      ...
      >
        <p
          ...
          style={{ color: DIM }}
        >
          Auto-Improvement Tracking — Maturation Loop
        </p>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr>
                {["Candidate", "Module", "Timestamp", "Outcome"].map((h) => (
                  <th
                    key={h}
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <p className="font-mono text-[8px]" style={{ color: DIM }}>
                    {relativeTime(a.created_at)} · score: {a.score.toFixed(0)}
                  </p>
                  {a.ai_review_summary && (
                    <p
