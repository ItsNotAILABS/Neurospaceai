...
const ALL_MODULE_CHECKS: Omit<
  ModuleCheckResult,
  "status" | "lastCheckedTs" | "criteriasMet"
>[] = [
  // Phase 1
  {
    id: "p1_schemas",
  ...
  },
  {
    ...
    subsystem: "regulation",
    recommendation:
      "Verify sympathetic/parasympathetic tones affect thresholds",
    doneCriteria: [
      "sym tone computed",
      "para tone computed",
      "thresholds shifted",
...
export function runAllChecks(
  ...
  return ALL_MODULE_CHECKS.map((check) => {
    ...
    const criteriasMet: boolean[] = check.doneCriteria.map((c) => {
...
export function calculateReadinessScore(checks: ModuleCheckResult[]): number {
  let totalWeight = 0;
  let weightedPass = 0;
...
export function getBlockers(checks: ModuleCheckResult[]): string[] {
  return checks
    .filter((c) => c.status === "fail" && c.blocking)
    .map((c) => c.name);
}

export function resolveDeploymentEligibility(
  ...
  for (let p = 1; p <= 9; p++) {
    ...
    phaseScores[`P${p}`] =
      phaseChecks.length > 0 ? passed / phaseChecks.length : 0;
  }
